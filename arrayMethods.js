...
let totalPop = populations.reduce((acc, curr) => {
  acc += curr;
  return acc;
});
...
const saleTotal = orders.map(total => parseFloat((total.price * (total.tax + 1)).toFixed(2)));
...
let totalCost = purchases.reduce((acc, curr) => {
  if (curr.owner.trim().toLowerCase() === "bob"){
  acc += curr.price;
  }
  return acc;
}, 0)
